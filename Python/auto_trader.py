...
def public_config_snapshot(config):
    ...
    return {
        ...
        "strategy": {
            "or_minutes": config.get("or_minutes"),
            "risk_reward": config.get("risk_reward"),
            "use_vwap": config.get("use_vwap"),
            "ema_length": config.get("ema_length"),
            "atr_length": config.get("atr_length"),
        },
        "broker": {
...
def parse_args():
    parser = argparse.ArgumentParser(description="VWAP ORB Pullback auto-trader")
    parser.add_argument("--symbol", help="Override symbol from config")
    parser.add_argument("--timeframe", help="Override timeframe from config (for example: 1m, 5m)")
    parser.add_argument("--poll-seconds", type=int, help="Override loop poll interval")
    parser.add_argument("--broker-name", choices=["paper", "alpaca", "ibkr"], help="Override broker name")
    parser.add_argument(
        "--dry-run",
        dest="dry_run",
        action="store_true",
        help="Force dry-run mode (no broker orders sent)",
    )
    parser.add_argument(
        "--live",
        dest="dry_run",
        action="store_false",
        help="Disable dry-run mode and allow live broker execution",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Stop after N processed bars; 0 means run forever",
    )
...
def apply_cli_overrides(config, args):
    ...
    if args.broker_name:
        config["broker"]["name"] = args.broker_name
...
def main():
    ...
    max_loops = max(0, int(args.max_loops))
    processed_bars = 0
    ...
    while True:
        ...
        if max_loops > 0 and processed_bars >= max_loops:
            logger.log("shutdown", reason="max_loops_reached", processed_bars=processed_bars)
            break
